...
export function WeeklyDistanceChart(props: Props) {
  ...
  return (
    <div
      style={{ width: "100%", height: isMobile ? "33vh" : 420 }}
      className=""
    >
          ...
          <Tooltip
            ...
            labelFormatter={(label) => {
              ...
              if (weekData?.weekStart && weekData?.weekEnd) {
                // Даты уже в формате DD.MM.YYYY, берем только DD.MM
                const formatDateStr = (dateStr: string) => {
                  const parts = dateStr.split('.');
                  return parts.length >= 2 ? `${parts[0]}.${parts[1]}` : dateStr;
                };
                return `Неделя ${label} (${formatDateStr(weekData.weekStart)} - ${formatDateStr(weekData.weekEnd)})`;
              }
              return `Неделя ${label}`;
            }}
          />
