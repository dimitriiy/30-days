...
const getIconByStatus = (type, day) => {
  const icons = {
    ...
    base: `<img
        width="150"
        height="150"
        src="img/abs_six_pack${day < 6 ? '_start' : ' (' + (31 - day) + ')'}.svg"
...
const createCard = ({ day, users, tasks }) => {
  ...
  const pic = {
    ...
    base: `<img
        width="150"
        height="150"
        src="img/abs_six_pack${day.day < 6 ? '_start' : ' (' + (31 - day.day) + ')'}.svg"
      />`,
  };
...
function createMinCard({ day, users, tasks }) {
  ...
  return `<div class="table-cards__item ${isDone && isPastDay ? 'table-cards__item--done' : ''} ${isToday ? 'table-cards__item--today animate__animated animate__pulse animate__infinite infinite' : ''}">   
        ...
        ${todayBadge}
    <span class="table-cards__item-pic">${getIconByStatus(picType, day.day)}</span>
<span class="table-cards__day">${day.day}</span></div>`;
}
function generateTableCards({ users, tasks }) {
  const cards = generateMonth({ year: getCurrentChallengeInfo().year, month: getCurrentChallengeInfo().month })
